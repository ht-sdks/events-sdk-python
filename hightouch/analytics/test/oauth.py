...
def mocked_requests_get(*args, **kwargs):
    ...
    if kwargs['url'] == 'http://127.0.0.1:80/token':
        return MockResponse(
            {'json_data': {'access_token': 'test_token', 'expires_in': 4000}}, 200
        )
    elif kwargs['url'] == 'http://127.0.0.1:400/token':
        return MockResponse(
            {
                'reason': 'test_reason',
                'json_data': {'error': 'unrecoverable', 'error_description': 'nah'},
            },
            400,
        )
        ...
        return MockResponse(
            {
                'reason': 'test_reason',
                'headers': {'X-RateLimit-Reset': time.time() * 1000 + 2000},
            },
            429,
        )
        ...
        return MockResponse(
            {
                'reason': 'test_reason',
                'json_data': {'error': 'recoverable', 'error_description': 'nah'},
            },
            500,
        )
        ...
        if mocked_requests_get.error_count < 0 or mocked_requests_get.error_count > 0:
            ...
            return MockResponse(
                {
                    'reason': 'test_reason',
                    'json_data': {'error': 'recoverable', 'message': 'nah'},
                },
                500,
            )
    ...
    elif kwargs['url'] == 'https://us-east-1.hightouch-events.com/v1/batch':
...
class TestOauthManager(unittest.TestCase):
    ...
    def test_oauth_rate_limit_delay(self, mock_sleep, mock_post):
        ...
        self.assertTrue(
            mock_sleep.call_args[0][0] > 1.9 and mock_sleep.call_args[0][0] <= 2.0
        )
